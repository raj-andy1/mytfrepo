...
provisioner "remote-exec" {
    inline = [
      "sudo curl -k https://ec2-52-38-176-168.us-west-2.compute.amazonaws.com:8140/packages/current/install.bash | sudo bash -s extension_requests:pp_role=demo-webserver extension_requests:pp_created_by=AndyR extension_requests:pp_department=TSE",
    ]
  connection {
    type = "ssh"
    user = "ec2-user"
